...
public interface IJobService : ITransientService
{
    ...
    string EnqueueAfter(string jobId, Expression<Action> methodCall);

    string EnqueueAfter(string jobId, Expression<Func<Task>> methodCall);

    string EnqueueAfter<T>(string jobId, Expression<Action<T>> methodCall);

    string EnqueueAfter<T>(string jobId, Expression<Func<T, Task>> methodCall);
