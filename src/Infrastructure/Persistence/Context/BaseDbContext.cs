...
using Cleanception.Infrastructure.Auditing;
...
public abstract class BaseDbContext : IdentityDbContext<ApplicationUser, ApplicationRole, string, IdentityUserClaim<string>, ApplicationUserRole, IdentityUserLogin<string>, ApplicationRoleClaim, IdentityUserToken<string>>
{
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        // If you want to see the sql queries that enfcore executes:
    ...
    public async Task<int> SaveChangesAsync(bool? withAuditing = true, CancellationToken cancellationToken = new CancellationToken())
    {
        var auditEntries = HandleAuditingBeforeSaveChanges(_currentUser.GetUserId(), withAuditing);
    ...
    private List<AuditTrail> HandleAuditingBeforeSaveChanges(Guid userId, bool? withAuditing = true)
    {
        string? userName = _currentUser.Name;
        if (withAuditing == true)
        {
...
public abstract class AcctivateBaseDbContext : DbContext
{
    protected readonly ICurrentUser _currentUser;
    private readonly ISerializerService _serializer;
    private readonly DatabaseSettings _dbSettings;
    ...
    protected AcctivateBaseDbContext( DbContextOptions options, ICurrentUser currentUser, ISerializerService serializer, IOptions<DatabaseSettings> dbSettings, IEventPublisher events)
        : base(options)
    {
        _currentUser = currentUser;
        _serializer = serializer;
