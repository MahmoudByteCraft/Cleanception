...
namespace Cleanception.Infrastructure.FileStorage;
...
public class LocalFileStorageService : IFileStorageService
{
    ...
    where T : class
    {
        ...
        string base64Data = Regex.Match(request.Data, "data:(?<fileType>.+?)/(?<type>.+?),(?<data>.+)").Groups["data"].Value;
    ...
    public async Task<string> UploadAsync<T>(IFormFile? file, FileType supportedFileType, CancellationToken cancellationToken = default)
    {
        ...
        if (!supportedFileType.GetDescriptionList().Contains(fileExtension))
            throw new InvalidOperationException("File Format Not Supported.");
